<template>
  <draggable ref="drag-board" class="drag-canvas-warp" :list="storeList" v-bind="getOptions" @add="onAdd">
    <template v-for="item in storeList">
      <drag :key="item.id" :default-x="item.position.clientX" :default-y="item.position.clientY" :aim-id="item.id" :update-id="item.updateId" :component-object="item" :is-instance="item.instance" :default="item.default" @resize-end="onResizeEnd" @move-end="onMoveEnd" />
    </template>
  </draggable>
</template>
